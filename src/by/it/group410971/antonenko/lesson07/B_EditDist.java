...
/*
Задача: Итерационно вычислить расстояние Левенштейна между двумя строками.
*/
...
public class B_EditDist {
    ...
    int getDistanceEdinting(String one, String two) {
        ...
        // Инициализация первой строки и первого столбца (пустая строка)
            ...
            for (int j = 1; j <= m; j++) {
                ...
                dp[i][j] = Math.min(
                        Math.min(dp[i - 1][j] + 1,    // удаление
                                dp[i][j - 1] + 1),   // вставка
                        dp[i - 1][j - 1] + cost       // замена (или отсутствие операции)
                );
            }
        }
